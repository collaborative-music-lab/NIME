...
def mirror(*args) -> None:
    #prototype function for receiving OSC messages
...
def keyMap(*args):
    ...
    #always sends keypresses in (row,column) format
    #- also remember the first argument sent to an OSC callback is the address
    # so this function gets the arguments <addres><row><col>
    print(args)
    row = args[1]
    col = args[2]
    address = '/pitch'
