...
#feel free to set a fixed tempo!
clockTempo = random.random()*20+100
clockInterval = 60/(clockTempo*4) #interval for 16th notes
...
async def loop():
    global clockInterval, prevTime, timeCounter
    ...
    while(t.check()): 
        ...
        if time.time() > (prevTime+clockInterval):
            prevTime = time.time()
            timeCounter += 1
