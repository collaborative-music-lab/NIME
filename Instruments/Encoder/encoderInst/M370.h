...
struct defDac{
  ...
  uint16_t sample; //current counter
  uint16_t inc; //increment to counter

  //sensor variables
  int16_t cur; //lerped data
  int16_t sensor; //data from a sensor
  byte smooth; //lerp smoothing factor
  
};

const  byte NUM_DACS = 2;
defDac dds[NUM_DACS];
