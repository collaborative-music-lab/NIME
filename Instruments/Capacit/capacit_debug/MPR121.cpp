...
MPR121::MPR121(byte _SDA, byte _SCL) :
  SDA(_SDA),
  SCL(_SCL) 
  {}
 ...
 */
boolean MPR121::begin(uint8_t i2caddr, TwoWire *theWire,
...
/*!
 *  @brief      DEPRECATED. Use MPR121::setThresholds(uint8_t touch,
 *              uint8_t release) instead.
 *  @param      touch
 *              see MPR121::setThresholds(uint8_t touch, uint8_t
 * *release)
 *  @param      release
 *              see MPR121::setThresholds(uint8_t touch, *uint8_t
 * release)
...
void MPR121::chargeCurrent(byte val){
 byte FFI = 0;
 val > 63 ? val = 63 : 0;
 val = (FFI<<6) + val;
...
void MPR121::chargeTime(byte val){
 byte SFI = 0;
 byte ESI = 0;
 val < 1 ? val=1: 0;
 val > 7 ? val = 7 : 0;
...
void MPR121::proxChargeCurrent(byte val){
 val > 63 ? val = 63 : 0;
 writeRegister(MPR121_PROXIMITY_CHARGECURRENT, val);
}

void MPR121::proxChargeTime(byte val){
 val > 7 ? val = 7 : 0;
