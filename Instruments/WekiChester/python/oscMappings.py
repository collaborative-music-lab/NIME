...
#mappings for WekiChester.py
######################
#INCOMING OSC
# handle messages sent from and wekinator
######################

enableIMUmonitoring = 1

def defineOscHandlers():
	dispatcher.map("/wek/outputs", processWeki)
...
def processWeki(a,b,c,d,e,f):
	...
	sendOSC("vca", 7, "CV", output[2]*127)
	sendOSC("vca", 8, "CV", output[2]*127)

	sendOSC("vca", 1, "VCA", output[3]*127)
	sendOSC("vca", 2, "VCA", output[4]*127)
	sendOSC("vca", 3, "VCA", output[5]*127)
	sendOSC("vca", 4, "VCA", output[3]*127)
...
def sendTiltToWeki():
	data = state['accelTilt'] + [0,0]
	#print("sending to weki", data)
	weki.send_message("/wek/inputs", data)
