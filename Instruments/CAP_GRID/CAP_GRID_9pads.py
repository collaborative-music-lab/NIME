...
def interpretMessage(message):
    ...
    #potentiometers
    if( message[0] == 27):
        address = '/wtable' # osc address
        val = (message[1]<<8) + message[2] #assembling an integer from two bytes
        msg = ['WTABLE_X', scale(val,0,4096,0,1,2),50] #scaling and assembling a packet
    ...
    #special inputs for CAP_GRID
        ...
        #remap to match physical layout
...
def setGridPitch(add, num):
    num = int(num)
    print('grid pitchset', add, num)
    if(num in range(len(gridPitches))):
        for i in range(12):
            address = '/pitchGrid'
            msg = ('cap-p-' + str(i+1) + '-r', (gridPitches[num][i]+1))
            client.send_message(address, msg)
...
dispatcher.map("/updateGridPitch", setGridPitch)
...
async def loop():
    ...
    setEnables()
    setCapSense()
    setGridPitch("/updateGridPitch", 1)
