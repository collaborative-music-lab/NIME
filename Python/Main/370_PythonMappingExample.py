...
def interpretMessage(message):
    ...
    if( message[0]==1): #messages mirrored from data sent to ESP32
...
def sensorMapping(address, val):
    if(address == OSC_INDEX_ARRAY[0]): #map analog input 0
        val = scale(val,0,4095,100,1000,curve=2)
        smoothing = 100
        outMsg = [val,smoothing]
        client.send_message('/testVal', outMsg)
...
#print('test',scale(127,0,1024,0,1,curve=2))
